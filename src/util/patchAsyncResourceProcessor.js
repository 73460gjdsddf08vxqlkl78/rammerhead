...
const {
  encodeContent,
  decodeContent,
} = require("testcafe-hammerhead/lib/processing/encoding");
...
function getResourceUrlReplacer(ctx) {
  return function urlReplacer(
    ...
    if (
      !urlUtil.isSupportedProtocol(resourceUrl) &&
      !urlUtil.isSpecialPage(resourceUrl)
    )
      return resourceUrl;
    ...
    const charsetStr =
      charsetAttrValue || (isScriptLike && ctx.contentInfo.charset.get());
...
require("testcafe-hammerhead/lib/processing/resources/index").process =
  async function process(ctx) {
    ...
    for (const processor of RESOURCE_PROCESSORS) {
      ...
      const processed = await processor.processResource(
        decoded,
        ctx,
        charset,
        urlReplacer,
