...
class RammerheadSessionFileCache extends RammerheadSessionAbstractStore {
  ...
  has(id) {
    return (
      this.cachedSessions.has(id) || fs.existsSync(this._getSessionFilePath(id))
    );
  }
  /**
   * @param {string} id
  ...
  get(id, updateActiveTimestamp = true, cacheToMemory = true) {
    ...
    if (this.cachedSessions.has(id)) {
      this.logger.debug(
        `(FileCache.get) returning memory cached session ${id}`,
      );
  ...
  add(id) {
    ...
    fs.writeFileSync(
      ...
      new RammerheadSession().serializeSession(),
    );
  ...
  _getSessionFilePath(id) {
    return path.join(
      this.saveDirectory,
      id.replace(/\/|\\/g, "") + sessionFileExtension,
    );
  }
  /**
   * @private
