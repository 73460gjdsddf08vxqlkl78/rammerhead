(function () {
  ...
  function main() {
    ...
    function getTimestamp() {
      ...
      if (isNaN(timestamp)) {
        if (rawTimestamp) {
          console.warn(
            "invalid timestamp retrieved from storage: " + rawTimestamp,
          );
        }
        return null;
      }
  ...
  function addUrlShuffling() {
    ...
    const sessionId = (location.pathname.slice(1).match(/^[a-z0-9]+/i) ||
      [])[0];
    if (!sessionId) {
    ...
    hammerhead.utils.url.overrideGetProxyUrl(function (url, opts) {
      ...
      return replaceUrl(
        getProxyUrl(url, opts),
        (u) => shuffler.shuffle(u),
        true,
      );
    });
    hammerhead.utils.url.overrideParseProxyUrl(function (url) {
      return parseProxyUrl(
        replaceUrl(url, (u) => shuffler.unshuffle(u), false),
      );
    });
    // manual hooks //
  ...
  function fixElementGetter() {
    ...
    const urlRewrite = (url) =>
      (hammerhead.utils.url.parseProxyUrl(url) || {}).destUrl || url;
  ...
  function fixCrossWindowLocalStorage() {
    ...
    const getLocHost = (win) =>
      new URL(hammerhead.utils.url.parseProxyUrl(win.location.href).destUrl)
        .host;
    const prefix = (win) =>
      `rammerhead|storage-wrapper|${
        hammerhead.settings._settings.sessionId
      }|${getLocHost(win)}|`;
        ...
        value: new Proxy(window[storageProp], {
          ...
          ownKeys() {
            ...
            for (const [key] of Object.entries(realStorage)) {
              ...
              if (proxyKey && !reservedProps.includes(proxyKey))
                list.push(proxyKey);
            }
            return list;
          },
    ...
    rewriteFunction("getItem", function (key) {
      return (
        this.internal.nativeStorage[toRealStorageKey(key, this.internal.ctx)] ||
        null
      );
    });
